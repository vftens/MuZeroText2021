...
function useHoistedState(initialState) {
    ...
    var _a = react_1["default"].useState(initialState), state = _a[0], _setState = _a[1];
...
function useSelect(params) {
    var _a;
    var options = params.options, onChange = params.onChange, scrollToIndex = params.scrollToIndex, optionsRef = params.optionsRef, controlRef = params.controlRef, _b = params.disabled, disabled = _b === void 0 ? false : _b, _c = params.multi, multi = _c === void 0 ? false : _c, getOptionLabel = params.getOptionLabel, onCreate = params.onCreate, getGroupOptions = params.getGroupOptions, onSelectOption = params.onSelectOption;
    var value = (_a = params.value) !== null && _a !== void 0 ? _a : [];
    var _d = useHoistedState(initialState), _e = _d[0], searchValue = _e.searchValue, isOpen = _e.isOpen, highlightedIndex = _e.highlightedIndex, resolvedSearchValue = _e.resolvedSearchValue, scrollToHighlighted = _e.scrollToHighlighted, setState = _d[1];
    var originalOptions = react_1.useMemo(function () {
        return typeof getGroupOptions === 'function'
            ? options
                ...
                return items.map(function (item) { return ({ label: getOptionLabel(item), item: item, group: groupName }); });
            })
                .flat()
    ...
    filterFnRef.current = function (options, searchValue) {
        if (typeof getOptionLabel === 'function') {
            ...
            var tempOptions = originalOptions
                .filter(function (option) { return option.label.toLowerCase().includes(searchValueLowerCase_1); })
                .sort(function (a) { return a.label.toLowerCase().indexOf(searchValueLowerCase_1); });
            var matchWithValueSearch = Boolean(originalOptions.find(function (option) { return option.label.toLowerCase() === searchValueLowerCase_1; }));
            var optionForCreate = {
                label: searchValue,
            ...
            return typeof onCreate === 'function' && !matchWithValueSearch
                ? tslib_1.__spreadArrays([optionForCreate], tempOptions) : tempOptions;
        }
    ...
    var getSelectedOptionIndex = function () {
        if (value) {
            var valuesArray = filteredOptions.map(function (option) { return option.item; });
    ...
    var selectIndex = react_1["default"].useCallback(function (index) {
        ...
        if (option && onChangeRef.current) {
            if (multi) {
                var newVal = value.some(function (v) { return getOptionLabel(v) === option.label; })
                    ? value.filter(function (v) { return getOptionLabel(v) !== option.label; })
                    : tslib_1.__spreadArrays(value, [option.item]);
                if (!option.optionForCreate) {
                    onChangeRef.current(newVal);
                }
                else if (typeof onCreateRef.current === 'function') {
                    var newValue = option.label;
